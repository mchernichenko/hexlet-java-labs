...
class AppTest {
    ...
    // Проверяем, что логи приложения записываются в файл log.txt
    // при входе пользователей в приложение
    @Test
    void testLogging() throws IOException, ParseException {
        CloseableHttpClient client = HttpClients.createDefault();

        String beforeLoginContent = getFileContent("log.txt");
        ...
        String afterLoginContent = getFileContent("log.txt");
    ...
    public static void tearDown() throws LifecycleException, IOException {
        app.stop();
        Path logPath = Paths.get("log.txt").toAbsolutePath().normalize();
