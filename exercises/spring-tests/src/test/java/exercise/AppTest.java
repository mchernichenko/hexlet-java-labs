...
@SpringBootTest
// При тестировании можно вообще не запускать сервер. То есть при выполнении тестов сетевое соединение не создается.
...
public class AppTest {
    ...
    // Внедрить эту зависимость возможно благодаря @AutoConfigureMockMvc
    ...
    void testPatchPerson() throws Exception {
        MockHttpServletResponse response = mockMvc.perform(patch("/people/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"id\": 1, \"firstName\": \"Mikhail\", \"lastName\": \"Chernichenko\"}")
                )
                .andReturn()
                .getResponse();
        ...
        // проверим, что данные c id=1 успешно обновились
        response = mockMvc
        ...
        assertThat(response.getStatus()).isEqualTo(200);
        assertThat(response.getContentType()).isEqualTo(MediaType.APPLICATION_JSON.toString());
        assertThat(response.getContentAsString()).contains("Mikhail", "Chernichenko");
    ...
    void testDeletePerson() throws Exception {
        ...
        // проверим, что данные c id=1 успешно удалились
        response = mockMvc
                .perform(get("/people/1"))
                .andReturn()
                .getResponse();
